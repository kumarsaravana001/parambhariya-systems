...
export function seedData() {
  ...
  const mkZone = (id: string, roomId: string, name: string, t: number, rh: number, co2: number) =>
    ({ id, roomId, name, setpointTempC: t, setpointRhPct: rh, setpointCo2Ppm: co2, tempC: null, rhPct: null, co2Ppm: null, lightLux: null, status: "OK", updatedAt: null });
  const zones = [
    mkZone("z-a1-c1", "r-a1", "Colonization Tunnel 1", 24, 88, 1400),
    mkZone("z-a2-f1", "r-a2", "Fruiting Chamber 1", 24, 90, 800),
    mkZone("z-a3-p1", "r-a3", "Pinning Bench 1", 20, 93, 650),
    mkZone("z-p1-c1", "r-p1", "Colonization Tunnel 1", 25, 86, 1500),
    mkZone("z-p2-f1", "r-p2", "Fruiting Chamber 1", 22, 89, 820),
  ];
  const bags = [
  ...
  ].map((b) => ({ id: uid(), code: b.code, strainId: b.strainId, zoneId: b.zoneId, status: b.status, stageProgress: b.stageProgress, weightG: b.weightG, createdAt: iso(b.age) }));
